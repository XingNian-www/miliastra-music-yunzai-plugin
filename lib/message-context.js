...
export function senderId(event = {}) {
  return firstIdentity(event.user_id, event.sender?.user_id)
}

export function senderMemoryKey(event = {}) {
  const adapter = firstIdentity(
    event.adapter,
    event.adapter?.name,
    event.adapter?.id,
  ...
  return `${adapter}:${selfId}:${senderId(event) || "unknown-user"}`
}
