...
const UNSAFE_EXTRA_BODY_KEYS = new Set(["__proto__", "constructor", "prototype"])
...
export async function optimizeTurtleSoup(request, ai = {}, fetchImpl, dependencies = {}) {
  ...
  try {
    const response = await transport.client.responses.create({
      ...settings.extraBody,
...
function validateExtraBody(value) {
  ...
  if (!isOrdinaryObject(value)) {
    throw new Error("海龟汤 AI extraBody 必须是普通 JSON 对象")
  }
  return cloneSafeJsonValue(value, "extraBody", new Set())
}

function cloneSafeJsonValue(value, path, seen) {
  ...
  if (typeof value !== "object") {
    throw new Error(`海龟汤 AI extraBody 仅支持 JSON 数据：${path}`)
  }
  if (seen.has(value)) {
    throw new Error(`海龟汤 AI extraBody 不能包含循环引用：${path}`)
  }
  ...
  try {
    if (Array.isArray(value)) {
      if (Object.getPrototypeOf(value) !== Array.prototype) {
        throw new Error(`海龟汤 AI extraBody 仅支持普通 JSON 数组：${path}`)
      }
      assertSafeArrayProperties(value, path)
      return value.map((item, index) => cloneSafeJsonValue(item, `${path}[${index}]`, seen))
    }
    if (!isOrdinaryObject(value)) {
      throw new Error(`海龟汤 AI extraBody 仅支持普通 JSON 对象：${path}`)
    }

    const result = {}
    for (const key of Reflect.ownKeys(value)) {
      if (typeof key === "symbol") {
        throw new Error(`海龟汤 AI extraBody 不能包含 Symbol 键：${path}`)
      }
      if (UNSAFE_EXTRA_BODY_KEYS.has(key)) {
        throw new Error(`海龟汤 AI extraBody 包含不安全键：${path}.${key}`)
      }
      const descriptor = Object.getOwnPropertyDescriptor(value, key)
      if (!descriptor?.enumerable || descriptor.get || descriptor.set) {
        throw new Error(`海龟汤 AI extraBody 仅支持普通数据属性：${path}.${key}`)
      }
      result[key] = cloneSafeJsonValue(descriptor.value, `${path}.${key}`, seen)
    }
    return result
  } finally {
    seen.delete(value)
...
function assertSafeArrayProperties(value, path) {
  for (const key of Reflect.ownKeys(value)) {
    if (key === "length") {
      continue
    }
    if (typeof key === "symbol" || !/^(0|[1-9]\d*)$/.test(key) || Number(key) >= value.length) {
      throw new Error(`海龟汤 AI extraBody 数组包含非 JSON 属性：${path}`)
    }
    const descriptor = Object.getOwnPropertyDescriptor(value, key)
    if (!descriptor?.enumerable || descriptor.get || descriptor.set) {
      throw new Error(`海龟汤 AI extraBody 仅支持普通数组元素：${path}[${key}]`)
    }
  }
  for (let index = 0; index < value.length; index += 1) {
    if (!Object.hasOwn(value, index)) {
      throw new Error(`海龟汤 AI extraBody 数组不能包含空位：${path}[${index}]`)
    }
  }
}
...
async function createAiTransport(settings, fetchImpl, dependencies) {
  ...
  return {
    client: createOpenAiClient(settings, {
      fetch: dependencies.proxyFetchImpl,
      httpAgent: agent
    }),
    close: () => agent?.destroy?.()
  }
