...
export function parseChatRelayCommand(message, options = {}) {
  const text = String(message || "")
  if (text.trim() === "#发言") {
    return { type: "switch" }
  }
  const prefixLength = options.requireDoublePrefix ? 2 : 1
  const prefix = text.slice(0, prefixLength)
  if (prefix.length !== prefixLength || !/^[!！]+$/.test(prefix)) {
    return null
  }
  const content = text.slice(prefixLength).replace(/\s+/g, " ").trim()
