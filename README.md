...
这是 Miliastra Wonderland Music 的配套 Yunzai 插件。它提供监控、受控启动、QQ 代发言、昵称白名单点歌和经过私聊确认的结构化海龟汤题目提交；不提供播放控制、队列修改或监听模式切换能力。
...
只有已经通过上述管理员命令备注发言昵称的 QQ 用户可以点歌：
...
`#点歌` 和 `#QQ点歌` 使用 QQ 音乐，`#网易点歌` 使用网易云，`#B站点歌` 使用哔哩哔哩。插件调用主程序的 `POST /searchPlay` 接口，并沿用主程序原生的“伴奏”关键词处理、点歌互斥、播放保护、任务去重和游戏内反馈。远程点歌按主程序语义显示为控制台来源；昵称映射只用于插件侧权限白名单，不会伪造主程序不支持的点歌者字段。

点歌与 QQ 代发言共用同一份 1 小时后端记忆。没有有效记忆时会先显示后端状态并要求选择，选择并成功入队后才记住；发送 `#发言` 切换后端也会改变后续点歌目标。昵称被删除后，该用户立即失去点歌权限。
...
插件仅允许只读访问 `/status`、`/monitor`、`/queue`、`/health`、`/screenshot`、`/turtle-soup` 和 `/undercover`。写接口只有 `/startup/game`、`/startup/enter-wonderland`、QQ 代发言使用的 `/chat/send`、昵称用户点歌使用的 `/searchPlay`，以及专用投稿客户端在确认后访问的 `/turtle-soup/questions`；其他会影响游戏的接口全部被插件拒绝。
