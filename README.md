...
插件默认记录所有对外网络请求，不提供关闭或修改保留周期的配置项。日志按天写入 `logs/network/network-YYYY-MM-DD.jsonl`，保留当天和此前六天；每天首次写入时自动删除更早的本插件网络日志。日志目录已被 Git 忽略，插件更新不会覆盖现有日志。

每条日志只包含时间、请求来源、HTTP 方法、去除查询参数和认证信息后的 URL、脱敏代理地址、HTTP 状态码、耗时、重试次数及网络错误类型。API 密钥、访问令牌、请求头、请求体、海龟汤内容、聊天内容、点歌关键词、查询参数值和响应正文均不会写入日志。日志写入或清理失败不会中断网络请求。
...
普通用户私聊发送 `!内容` 或 `！内容` 可以把内容加入游戏内发言队列，触发符不区分全角和半角；群聊必须发送 `!!内容`、`！！内容` 或混用全角半角的两个感叹号。群聊中的单感叹号不会被本插件处理。首次发送会显示各后端状态并要求回复数字；选择成功且发言入队后，插件会在内存中为该 QQ 用户记住后端 1 小时。同一用户在不同 QQ 群继续发言时会复用该选择，插件重启或配置热重载后记忆清空。
...
`#千星监控` 会读取播放控制器、运行状态、音乐队列、正式任务生命周期、聊天监听、海龟汤和脱敏卧底状态。后端仍只有旧版 `pendingTasks` 时，插件会自动回退到旧任务标签。`#千星启动原神` 和 `#千星进入千星` 始终会先显示后端状态，并要求回复数字确认一个后端；`#千星截图` 仅在配置多个后端时要求选择。游戏未运行或仍在启动、后端尚无可用主扫描画面时，截图命令会直接提示暂时无法获取截图。
